...
<div class="span12">
    <table class="table table-striped">
    	<thead>
        	<tr>
            	<td width=80px>名称</td>
                <td width=160px>简介</td>
                <td width=80px>时间</td>
                <td width=100px>承办方</td>
            </tr>
        </thead>
      <tbody>
      <?php if(is_array($activity)): $i = 0; $__LIST__ = $activity;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
        	<td><?php echo ($v['aname']); ?></td>
        	<td><?php echo ($v['acontent1']); ?> </td>
          <td><?php echo (date('Y-m-d D',$v['acontent2'])); ?></td>
          <td><?php echo a_getcname($v['cid']);?> </td>
          <?php if($_SESSION['user_power'] > 49): ?><td><a href="<?php echo U('/Activity/edit?id='.$v['aidentifier']);?>">修改</a></td><?php endif; ?>
