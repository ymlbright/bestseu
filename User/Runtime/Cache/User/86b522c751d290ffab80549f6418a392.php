...
<div class="page-header">
    ...
    <table class="table table-striped">
      ...
      <?php if(is_array($signup)): $i = 0; $__LIST__ = $signup;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$s): $mod = ($i % 2 );++$i;?><tr>
        	...
        	<td >社团报名表</td>
            <td >
              <?php switch($s['sstatus']): case "0": ?><a href="<?php echo U('/Community/SignUpDetial?id='.$s['cid']);?>">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('/Community/DelSignUp?id='.$s['cid']);?>">删除</a><?php break;?>
                <?php case "1": ?><a href="<?php echo U('/Community/SignUpDetial?id='.$s['cid']);?>">查看</a>(已通过)<?php break;?>
                <?php case "2": ?><a href="<?php echo U('/Community/SignUpDetial?id='.$s['cid']);?>">查看</a>(已拒绝)<?php break; endswitch;?>
            </td>
        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
       <?php if(is_array($approval)): $i = 0; $__LIST__ = $approval;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$a): $mod = ($i % 2 );++$i;?><tr>
        	<td >{<?php echo ($a['acontent1']); ?>)</td>
            <td ><?php echo (date('Y-m-d D',$a['adate'])); ?></td>
        	<td >社团成立申请表</td>
            <td >
              <?php switch($a['astatus']): case "0": ?><a href="#">删除</a><?php break;?>
                <?php case "1": ?><a href="#">查看</a>(已通过)<?php break;?>
                <?php case "2": ?><a href="#">查看</a>(已拒绝)<?php break; endswitch;?>
            </td>
        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
      </tbody>
    </table>
